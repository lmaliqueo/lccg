...
{!! Form::open(['route' => 'ensayos.psu.store', 'method'=>'POST', 'class'=>'ui form animated fadeIn']) !!}
			...
			<table class="ui celled table">
				<thead>
					<tr>
						...
						<td style="width: 25%">
                            <div class="field required">
                                {!! Form::select('ensayo[materia_id]', $psu->materias->pluck('mens_nombre', 'mens_id'), null, ['placeholder'=>'', 'required', 'class'=>'ui dropdown fluid']) !!}
    ...
    <div class="inline fields no-margin">
        ...
        <div class="field">
                <div class="ui search fluid focus" data-tipo="nombre">
                    <div class="ui icon input">
                        <input class="prompt persona" type="text" placeholder="Nombre Alumno" autocomplete="off" name="alumno[nombre]" size="35">
                        <i class="search icon inverted red circular remove link icon_remove" id="icon_person" style="display: none;" value="alumno"></i>
                    </div>
                </div>
        </div>
...
<table class="table ui celled" style="display: none;">
	...
	<table class="table ui celled navy2 table_puntajes" style="display: none;">
		<thead>
			<tr>
				<th>N°</th>
				<th>Rut</th>
    ...
    <div class="text-center">
        {!! Form::submit('Guardar', ['class'=>'ui button teal disabled', 'id'=>'guardar_ensayo']) !!}
    </div>
</div>
...
$('.ui.search').search({
    ...
    onSelect: function(result, response){
        ...
        if(tipo == 'rut'){
            $('input[name="alumno[nombre]"]').val(result.nombre);
        }else{
...
{{-- 
 ...
 --}}
    $('.add_alumno').on('click', function(){
        ...
        //document.getElementsByClassName("add_alumno").setAttribute('cont', count)
